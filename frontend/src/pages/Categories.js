...
const Categories = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (isEditMode) {
        await api.put(`/api/posts/categories/${selectedCategory.id}`, formData);
      } else {
        await api.post('/api/posts/categories', formData);
      }
      handleCloseModal();
      fetchCategories();
    } catch (err) {
      alert(err.message || (isEditMode ? t('categories.failedToUpdate') : t('categories.failedToCreate')));
    }
  };
  ...
  const handleDelete = async (categoryId) => {
    ...
    try {
      await api.delete(`/api/posts/categories/${categoryId}`);
      fetchCategories();
    } catch (err) {
      alert(err.message || t('categories.failedToDelete'));
    }
  };
  ...
  if (!isEditorOrAdmin) {
    return (
      <div className="flex items-center justify-center min-h-screen">
        <div className="text-center">
          <p className="text-red-500">{t('categories.editorRequired')}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex flex-col gap-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">{t('categories.description')}</p>
        </div>
        <button
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-gray-50 dark:bg-[#1a1f28]">
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wider">{t('categories.name')}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wider">{t('categories.slug')}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wider">{t('categories.description')}</th>
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wider">{t('categories.createdAt')}</th>
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-600 dark:text-gray-400 uppercase tracking-wider">{t('categories.actions')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-200 dark:divide-[#3a3f4a]">
              {filteredCategories.length === 0 ? (
                ...
                currentCategories.map((category) => (
                  <tr key={category.id} className="hover:bg-gray-50 dark:hover:bg-[#1a1f28] transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-700 dark:text-gray-300">{category.description || '-'}</div>
                    </td>
        ...
        {filteredCategories.length > 0 && (
          <div className="bg-gray-50 dark:bg-[#1a1f28] px-6 py-4 flex items-center justify-between border-t border-gray-200 dark:border-[#3a3f4a]">
            <div className="text-sm text-gray-600 dark:text-gray-400">
              {t('tables.showing')} <span className="font-medium text-gray-900 dark:text-white">{startIndex + 1}</span> {t('tables.to')} <span className="font-medium text-gray-900 dark:text-white">{Math.min(endIndex, filteredCategories.length)}</span> {t('tables.of')}{' '}
              <span className="font-medium text-gray-900 dark:text-white">{filteredCategories.length}</span> {t('tables.entries')}
            </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white dark:bg-[#282e39] rounded-lg p-6 w-full max-w-md border border-gray-200 dark:border-[#3a3f4a]">
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white mb-4">
              {isEditMode ? t('categories.editCategory') : t('categories.newCategory')}
            </h2>
